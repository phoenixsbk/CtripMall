...
public class CtripResources {
    ...
    public ResultMsg postOrderInfoToCtrip(@PathParam("uuid") String uuid,
	    				  ...
	    				  @FormParam("flowticketnumber") String flowTicketNumber,
	    				  @FormParam("flowstatus") int flowStatus,
	    				  @FormParam("flowremark") String flowRemark) {
	final String METHOD = "postOrderInfoToCtrip";
	if (LOGGER.isLoggable(Level.FINER)) {
	    LOGGER.entering(CLASSNAME, METHOD, "[TRACE ORDER FLOW] Update the order:" + uuid +
		    " with flow:[" + flowCompanyName + "] [" + flowTicketNumber + "] [" + flowStatus + "] [" + flowRemark + "]");
	}

	ResultMsg msg = new ResultMsg();

	if (flowCompanyName == null || flowCompanyName.isEmpty() || flowTicketNumber == null || flowTicketNumber.isEmpty() || flowRemark == null) {
	    msg.setResult(1);
